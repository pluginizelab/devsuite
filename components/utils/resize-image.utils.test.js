import { calculateCropDimensions, handleResizeImage, isPointInCropRect, processImageFile, resizeImage, updateHeight, updateWidth, } from "./resize-image.utils";
describe("Image Processing Functions", () => {
    ...
    it("should reject if canvas context is not available", async () => {
        ...
        const img = { width: 1000, height: 500 };
        await expect(resizeImage({ img, format: "png" })).rejects.toThrow("Canvas context is not available");
    });
