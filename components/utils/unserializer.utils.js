...
class Parser {
  ...
  readBool() {
    ...
    const parsed = { type: "bool", value: val === "1" };
    this.parsedValues.push(parsed);
    return parsed;
  }
  ...
  readUtf8ByteLengthString(byteLength) {
    const start = this.pos;
    let consumedBytes = 0;
...
export function unserialize(input) {
  if (!input || input.trim() === "") {
...
export function isValidSerialized(input) {
  try {
    unserialize(input);
    return true;
  }
...
export function formatPrintR(value, indent = 0) {
...
export function formatVarDump(value, indent = 0) {
