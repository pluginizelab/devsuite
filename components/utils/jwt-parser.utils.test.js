...
describe("decodeJWT", () => {
    it("should decode a valid JWT", () => {
        ...
        const payload = Buffer.from(JSON.stringify({ sub: "1234567890", name: "John Doe", admin: true })).toString("base64url");
    ...
    it("should throw an error if JWT is missing parts", () => {
        const token = "eyJhbGciOiAiSFMyNTYiLCAidHlwIjogIkpXVCJ9.eyJzdWIiOiAiMTIzNDU2Nzg5MCIsIG5hbWUiOiAiSm9obiBEb2UiLCAiYWRtaW4iOiB0cnVlfQ";
        expect(() => decodeJWT(token)).toThrow("Invalid token");
    });
    it("should throw an error for a JWT with invalid Base64 encoding", () => {
        const token = "eyJhbGciOiAiSFMyNTYiLCAidHlwIjogIkpXVCJ9.invalid_payload.abc123";
        expect(() => decodeJWT(token)).toThrow("Invalid token");
    });
});
