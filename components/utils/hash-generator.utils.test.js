...
describe("generateHash", () => {
    ...
    it("should throw an error if data is an empty string", () => {
        expect(() => generateHash("sha256", "")).toThrow("Data must be a non-empty string");
    });
    it("should throw an error if data is not a string", () => {
        expect(() => generateHash("sha256", 123)).toThrow("Data must be a non-empty string");
    });
    it("should throw an error for unsupported algorithm", () => {
        expect(() => generateHash("unsupportedAlgo", "test")).toThrow("Failed to generate hash:");
    });
    it("should throw an unknown error if a non-error is thrown", () => {
        ...
        expect(() => generateHash("sha256", "test")).toThrow("Failed to generate hash: An unknown error occurred");
