...
describe("json-to-tsv.utils", () => {
    ...
    const normalizeTSV = (tsv) => tsv.replace(/\r\n/g, "\n").trim();
    it("should convert a JSON string to TSV", () => {
        const jsonString = '[{"name":"John","age":30},{"name":"Jane","age":25}]';
        const expectedTSV = "name\tage\nJohn\t30\nJane\t25";
        expect(normalizeTSV(convertJSONtoTSV(jsonString))).toBe(normalizeTSV(expectedTSV));
    });
    it("should convert a JSON object to TSV", () => {
        const jsonObject = { name: "John", age: 30 };
        const expectedTSV = "name\tage\nJohn\t30";
        expect(normalizeTSV(convertJSONtoTSV(jsonObject))).toBe(normalizeTSV(expectedTSV));
    });
    it("should convert an array of JSON objects to TSV", () => {
        ...
        const expectedTSV = "name\tage\nJohn\t30\nJane\t25";
        expect(normalizeTSV(convertJSONtoTSV(jsonArray))).toBe(normalizeTSV(expectedTSV));
    });
    ...
    it("should throw an error for invalid input", () => {
        expect(() => convertJSONtoTSV(123)).toThrow("Input must be a JSON string or an object.");
    });
});
