...
export const PRESET_PATTERNS = [
    ...
    },
    {
        ...
        testString: "Visit https://example.com/path?query=1\nOr http://sub.domain.org/page#section",
    },
    {
        name: "Phone (US)",
...
export const createRegex = (pattern) => {
    ...
    if (pattern.startsWith("/")) {
        ...
        if (pattern.includes("/")) {
            throw new Error("Invalid regex: slash (/) found within the pattern. If you intended to use regex delimiters, ensure the pattern starts and ends with a slash.");
        }
        patternBody = pattern;
    }
    try {
...
export const parseFlags = (pattern) => {
    const flags = {
        g: false,
        i: false,
        m: false,
...
export const buildPatternWithFlags = (patternBody, flags) => {
...
export const getPatternBody = (pattern) => {
    if (pattern.startsWith("/")) {
        const lastSlashIndex = pattern.lastIndexOf("/");
        if (lastSlashIndex > 0) {
...
export const findCaptureGroups = (pattern) => {
...
export const explainPattern = (pattern) => {
    const patternBody = getPatternBody(pattern);
    const components = [];
    let i = 0;
    const explanations = {
...
export const getMatchStats = (pattern, testString) => {
...
export const getDetailedMatches = (pattern, testString) => {
    const matches = [];
    try {
        const regex = createRegex(pattern);
        const globalRegex = new RegExp(regex.source, regex.flags.includes("g") ? regex.flags : regex.flags + "g");
