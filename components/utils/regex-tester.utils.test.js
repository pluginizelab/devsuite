...
describe("createRegex", () => {
    ...
    test("should throw error for non-string input", () => {
        expect(() => createRegex(123)).toThrow("Pattern must be a non-empty string");
    });
    ...
    test("should throw error for pattern with duplicate flags", () => {
        expect(() => createRegex("/test/gg")).toThrow("Duplicate flags are not allowed: gg");
    });
    test("should throw error for pattern with missing closing slash", () => {
        expect(() => createRegex("/test")).toThrow("Invalid regex: missing closing slash");
    });
    test("should throw error for pattern with invalid regex syntax", () => {
        expect(() => createRegex("/[a-z/")).toThrow("Invalid regular expression: /[a-z/: Unterminated character class");
    });
