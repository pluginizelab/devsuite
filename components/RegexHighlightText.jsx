...
export default function RegexHighlightText(props) {
    ...
    props.matches.forEach((match, index) => {
        ...
        if (offset > lastIndex) {
            parts.push(<span key={`text-${lastIndex}-${offset}`}>
        ...
        parts.push(<HoverCard key={`match-${offset}-${index}`} openDelay={100} closeDelay={100}>
        ...
        <HoverCardContent side="top" className="w-auto max-w-xs p-3">
          <div className="space-y-1">
            ...
            {match.length <= 50 && (<p className="text-xs bg-muted px-1 py-0.5 rounded mt-1">
                &quot;{match === "\n" ? "\\n" : match}&quot;
              </p>)}
          </div>
        </HoverCardContent>
    ...
    if (lastIndex < props.text.length) {
        parts.push(<span key={`text-${lastIndex}-${props.text.length}`}>
