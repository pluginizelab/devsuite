...
const SVGUploadComponent = ({ onSVGSelect, maxFileSize = MAX_FILE_SIZE, }) => {
    ...
    const validateSVGFile = useCallback((file) => {
    ...
    const readSVGFile = useCallback((file) => {
    ...
    return (<div onDrop={handleDrop} onDragOver={handleDragOver} onDragLeave={handleDragLeave} onClick={handleClick} className="flex flex-col border border-dashed border-border p-6 text-center text-muted-foreground rounded-lg min-h-40 items-center justify-center bg-muted cursor-pointer mb-2">
      <input ref={inputRef} type="file" accept=".svg,image/svg+xml" className="hidden" onChange={handleFileChange}/>
      <UploadIcon />
...
const statusComponents = {
    idle: (maxSize) => (<StatusComponent title="Drag and drop your SVG file here, or click to select" message={`Max size ${maxSize}`}/>),
    loading: () => <StatusComponent title="Loading..."/>,
    error: (maxSize) => (<StatusComponent title="SVG file is too big!" message={`${maxSize} max`}/>),
