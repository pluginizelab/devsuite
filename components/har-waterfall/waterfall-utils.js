...
export function getTimingColor(type) {
    return TIMING_COLORS[type];
}
export function calculateTimings(entries) {
    if (entries.length === 0)
        return [];
    // Find the earliest start time
    const earliestStartTime = Math.min(...entries.map((entry) => new Date(entry.startedDateTime).getTime()));
    return entries.map((entry) => {
        const startTime = new Date(entry.startedDateTime).getTime() - earliestStartTime;
...
export function getRequestTypeColor(mimeType) {
