 ...
 */
export default function MatchIndicators({ categories, className = "", }) {
    ...
    return (<div className={`flex items-center gap-1 ${className}`}>
      {categories.map((category) => (<div key={category} className={`w-2 h-2 rounded-full ${getCategoryColor(category)}`} title={getCategoryTitle(category)}/>))}
    </div>);
}
