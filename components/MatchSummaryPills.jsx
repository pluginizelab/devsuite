import { getMatchCategories, } from "@/components/utils/har-utils";
/**
 ...
 */
export default function MatchSummaryPills({ entries, searchQuery, className = "", }) {
    if (!searchQuery) {
        return null;
    }
    ...
    return (<div className={`flex flex-wrap gap-2 ${className}`}>
      {activeCategories.map((cat) => (<div key={cat.category} className={`inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-medium ${cat.color}`}>
          {cat.label}: {cat.count}
        </div>))}
    </div>);
}
