...
import { calculateCropDimensions, handleResizeImage, isPointInCropRect, updateHeight, updateWidth, } from "@/components/utils/resize-image.utils";
...
const getImageRect = (imageElement) => {
    if (!imageElement) {
        return null;
    }
...
const getPointerPositionInRect = (clientX, clientY, rect) => ({
    x: clamp(clientX - rect.left, 0, rect.width),
    y: clamp(clientY - rect.top, 0, rect.height),
});
const areCropRectsEqual = (left, right) => left.x === right.x &&
    left.y === right.y &&
...
export default function ImageResize() {
    ...
    const [resizedImageInfo, setResizedImageInfo] = useState({});
    const [isCropping, setIsCropping] = useState(false);
    const [cropRect, setCropRect] = useState(null);
    const imageRef = useRef(null);
    const animationTimeoutRef = useRef(null);
    const outputObjectUrlRef = useRef(null);
    const cropInteractionRef = useRef(null);
    const cropFrameRef = useRef(null);
    const pendingCropRectRef = useRef(null);
    ...
    const setOutputAndShowAnimation = useCallback((output) => {
        setOutput(output);
    ...
    const updateResizedImageInfo = useCallback((width, height, format, quality) => {
        setResizedImageInfo({
    ...
    const commitTransformedOutput = useCallback((nextOutput) => {
        releaseOutputObjectUrl();
    ...
    const handleQualityInput = useCallback((e) => {
        const parsedValue = Number(e.target.value);
        if (Number.isNaN(parsedValue)) {
            return;
        }
    ...
    const createInitialCropRect = useCallback((imageWidth, imageHeight) => {
        ...
        const cropWidth = clamp(imageWidth - horizontalInset * 2, minWidth, imageWidth);
        const cropHeight = clamp(imageHeight - verticalInset * 2, minHeight, imageHeight);
        return {
    ...
    const getCropHandleFromTarget = useCallback((target) => {
    ...
    const queueCropRectUpdate = useCallback((nextRect) => {
    ...
    const handlePointerDown = useCallback((e) => {
        ...
        const position = getPointerPositionInRect(e.clientX, e.clientY, imageRect);
        e.preventDefault();
    ...
    const applyCroppedOutput = useCallback((croppedDataUrl, cropWidth, cropHeight) => {
        ...
        updateResizedImageInfo(normalizedWidth, normalizedHeight, format, quality);
        resetCropStates();
    }, [
        format,
        quality,
    ...
    const cropSvgImage = useCallback((img, x, y, width, height) => {
    ...
    const cropCanvasImage = useCallback((img, x, y, width, height) => {
        ...
        if (ctx) {
            ...
            ctx.drawImage(img, Math.round(x), Math.round(y), normalizedWidth, normalizedHeight, 0, 0, canvas.width, canvas.height);
            canvas.toBlob((blob) => {
                if (!blob) {
                    return;
                }
    ...
    const handleApplyCrop = useCallback(() => {
        ...
        const { x, y, width, height } = calculateCropDimensions(currentImageRef, currentImageRef, activeCropRect);
        if (width < 1 || height < 1) {
            return;
        }
        if (format === "svg") {
    ...
    return (<main>
      ...
      <section className="container max-w-2xl mb-12">
        <PageHeader title="Image Resizer" description="Fast, free, open source, ad-free tools."/>
      </section>

      <section className="container max-w-2xl mb-6">
        <Card className="flex flex-col p-6 hover:shadow-none shadow-none rounded-xl">
          <div>
            <ImageUploadComponent maxFileSize={MAX_FILE_SIZE} onFileSelect={handleFileSelect}/>

            <div className="flex justify-between items-center mb-2 pt-6">
              <div className="flex-1 mr-2">
                <Label className="mb-2">Width (px)</Label>
                <Input type="number" placeholder="Enter width" onChange={handleWidthChange} value={width ?? ""} className="mb-2" disabled={!imageFile || isCropping}/>
              </div>
              <div className="flex-1 ml-2">
                <Label className="mb-2">Height (px)</Label>
                <Input type="number" placeholder="Enter height" onChange={handleHeightChange} value={height ?? ""} className="mb-2" disabled={!imageFile || isCropping}/>
              </div>
            </div>

            <div className="flex items-center mb-6 gap-2">
              <Checkbox id="preserve-aspect-ratio" checked={preserveAspectRatio} onCheckedChange={handleAspectRatioChange} disabled={!imageFile || isCropping} className="mr-1"/>
              <Label htmlFor="preserve-aspect-ratio" className="mb-0 hover:cursor-pointer">
            ...
            <div className="mb-6 flex w-full gap-4">
              <div className="flex flex-1 flex-col">
                ...
                <Combobox data={formatOptions} value={format} onSelect={handleFormatChange} disabled={!imageFile || isCropping}/>
              </div>

              {format === "jpeg" && (<div className="flex flex-1 flex-col">
                  <Label>Quality (0.1 to 1.0)</Label>
                  <Input type="number" min="0.1" max="1.0" step="0.1" value={quality} onChange={handleQualityInput} disabled={!imageFile} className="h-[32px] rounded-md"/>
                </div>)}
            </div>

            <DividerComponent margin="medium"/>

            <div className={cn(imageFile && "mb-4", "flex w-full flex-wrap gap-3")}>
              {!isCropping && (<Button className="flex flex-1 min-w-[140px]" onClick={handleCropModeToggle} disabled={!output} variant="outline">
                  <Crop className="h-4 w-4 mr-2"/>
                  Crop Mode
                </Button>)}

              {isCropping && (<Button className="flex flex-1 min-w-[140px]" onClick={handleCropModeToggle} variant="outline">
                  Exit Crop Mode
                </Button>)}

              <Button className="flex flex-1 min-w-[140px]" onClick={handleResizeAndDownload} disabled={!imageFile || isCropping}>
            ...
            {output && (<>
                ...
                <div>
                  ...
                  <div className="relative flex flex-col flex-1 items-center ring-1 ring-border rounded-lg p-1">
                    <div className="relative inline-block rounded-sm" onPointerDown={handlePointerDown} onPointerMove={handlePointerMove} onPointerUp={handlePointerUp} onPointerCancel={handlePointerCancel} style={isCropping ? { touchAction: "none" } : undefined}>
                      <img src={output} alt="Resized output" ref={imageRef} decoding="async" draggable={false} className={`block w-auto h-auto max-h-[640px] rounded-sm select-none ${showAnimation ? "animate-grow-from-center" : ""}`}/>
                      {isCropping && cropRect && (<CropOverlayComponent cropRect={cropRect} onCancel={handleCropModeToggle} onDone={handleApplyCrop}/>)}
                    </div>
                  </div>
                </div>
              </>)}
