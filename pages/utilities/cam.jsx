...
const getPermissionDeniedError = () => ({
    type: "permission_denied",
    title: "Camera Access Denied",
    message: "You've denied camera access. To use this feature, you'll need to enable camera permissions in your browser.",
    instructions: [
...
const getNotSupportedError = () => ({
    type: "not_supported",
    title: "Camera Not Supported",
    message: "Your browser doesn't support camera access. This feature requires a modern browser with MediaDevices API support.",
    instructions: [
...
const getPipNotSupportedError = () => ({
    type: "pip_not_supported",
    title: "Picture-in-Picture Not Supported",
    message: "Your browser doesn't support Picture-in-Picture mode. You can still use the camera preview on this page.",
    instructions: [
...
const getStreamError = (errorMessage) => ({
    type: "stream_error",
    title: "Camera Error",
...
export default function CameraUtility() {
    ...
    const mirroredStreamRef = useRef(null);
    const animationFrameRef = useRef(null);
    useEffect(() => {
        setIsPipSupported(typeof document !== "undefined" &&
            "pictureInPictureEnabled" in document &&
            document.pictureInPictureEnabled);
        setIsMediaSupported(typeof navigator !== "undefined" &&
    ...
    const createMirroredStream = useCallback((originalStream) => {
        ...
        const drawMirroredFrame = () => {
            if (!sourceVideoRef.current || !canvasRef.current)
                return;
            ctx.save();
            ctx.scale(-1, 1);
    ...
    const startCamera = useCallback(async () => {
        ...
        try {
            ...
            if (error.name === "NotAllowedError" ||
                ...
                setErrorInfo(getStreamError("No camera found. Please connect a camera and try again."));
            }
        ...
        try {
            ...
            if (error.name === "NotAllowedError") {
                setErrorInfo({
                    ...
                    message: "Picture-in-Picture was blocked. This usually happens if the request wasn't triggered by a user action.",
    ...
    return (<main>
      <Meta title="Camera with Picture-in-Picture | Free, Open Source & Ad-free" description="Use your camera with Picture-in-Picture mode for a floating camera overlay that stays on top of all applications."/>
      <Header />
      <CMDK />

      <section className="container max-w-2xl mb-10">
        <PageHeader title="Camera with Picture-in-Picture" description="A floating camera overlay that stays on top of all applications."/>
      </section>

      <section className="container max-w-2xl mb-12">
        <Card className="flex flex-col p-6 hover:shadow-none shadow-none rounded-xl">
          {status === "idle" && (<div className="flex flex-col items-center justify-center py-12">
          ...
          {status === "requesting" && (<div className="flex flex-col items-center justify-center py-12">
          ...
          {status === "error" && errorInfo && (<div className="flex flex-col py-6">
              <div className="bg-destructive/10 border border-destructive/20 rounded-lg p-6 mb-6">
                ...
                <div className="space-y-2">
                  ...
                  <ul className="list-disc list-inside space-y-1 text-sm text-muted-foreground">
                    {errorInfo.instructions.map((instruction, index) => (<li key={index}>{instruction}</li>))}
                  </ul>
                </div>
              </div>
          ...
          {(status === "active" || status === "pip") && (<div className="flex flex-col">
              <div className={cn("relative rounded-lg overflow-hidden bg-black mb-6", status === "pip" && "opacity-50")}>
                <video ref={videoRef} autoPlay playsInline muted className="w-full aspect-video object-cover"/>
                {status === "pip" && (<div className="absolute inset-0 flex items-center justify-center">
              ...
              <div className="flex flex-wrap gap-3 justify-center">
                {status === "active" && (<>
                    <Button onClick={enablePip} disabled={!isPipSupported} title={!isPipSupported
              ...
              {!isPipSupported && status === "active" && (<p className="text-sm text-muted-foreground text-center mt-4">
