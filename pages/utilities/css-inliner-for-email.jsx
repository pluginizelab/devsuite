...
export default function CSSInlinerForEmail() {
    ...
    const handleHtmlChange = useCallback((event) => {
        setHtmlInput(event.currentTarget.value);
    }, []);
    const handleCssChange = useCallback((event) => {
    ...
    const handleConvert = useCallback(() => {
        ...
        const errorMessage = conditions.find((value) => value.condition)?.message ?? "";
        try {
    ...
    return (<main>
      <Meta title="CSS Inliner for Email | Free, Open Source & Ad-free" description="Convert CSS styles to inline styles directly in your HTML with DevSuite's free CSS Inliner for Email. Just paste your HTML and CSS, and get the inlined HTML result."/>
      <Header />
      <CMDK />

      <section className="container max-w-2xl mb-12">
        <PageHeader title="CSS Inliner for Email" description="Convert your CSS to inline styles quickly and easily."/>
      </section>
      ...
      <section className="container max-w-2xl mb-6">
        <Card className="flex flex-col p-6 hover:shadow-none shadow-none rounded-xl">
          <div>
            <Label>HTML</Label>
            <Textarea rows={6} placeholder="Paste HTML here" onChange={handleHtmlChange} className="mb-6" value={htmlInput}/>

            <Label>CSS</Label>
            <Textarea rows={6} placeholder="Paste CSS here" onChange={handleCssChange} className="mb-4" value={cssInput}/>
