...
const outlineStyles = "outline outline-2 outline-green-500 outline-offset-2 shadow-md transition";
export default function InternetSpeedTest() {
    ...
    return (<main>
      <Meta title="Internet Speed Test | Free, Open Source & Ad-free" description="Run a fast, privacy-friendly internet speed test (download, upload, latency & jitter) powered by Cloudflare."/>
      <Header />
      <CMDK />

      <section className="container max-w-4xl mb-10">
        <PageHeader title="Internet Speed Test" description="Open source, ad-free speed test using Cloudflare edge."/>
      </section>

      <section className="container max-w-4xl mb-12 space-y-8">
        <div className="grid gap-6 md:grid-cols-4 auto-rows-[180px]">
          <Card className={cn("md:col-span-2 row-span-1 flex flex-col relative transition-shadow p-6", testState.status === "finished" && outlineStyles)}>
            <Label isRunning={testState.status === "running"} title="Download"/>

            <div className="flex-1 flex flex-col justify-end">
              <div className="text-5xl font-semibold tabular-nums leading-none">
                {((testState.result.download || 0) / 1000000).toFixed(2)}
          ...
          <Card className={cn("md:col-span-2 row-span-1 flex flex-col relative transition-shadow p-6", testState.status === "finished" && outlineStyles)}>
            ...
            <div className="flex-1 flex flex-col justify-end">
              <div className="text-5xl font-semibold tabular-nums leading-none">
                {((testState.result.upload || 0) / 1000000).toFixed(2)}
                <span className="ml-2 text-base font-normal text-muted-foreground">
                  Mbps
                </span>
              </div>
          ...
          <Card className={cn("md:col-span-1 row-span-1 flex flex-col transition-shadow p-6", testState.status === "finished" && outlineStyles)}>
          ...
          <Card className="md:col-span-2 row-span-1 flex flex-col cursor-pointer select-none group border-none bg-foreground" onClick={toggleTest} role="button" tabIndex={0} onKeyDown={(e) => {
...
const PulsatingCircle = () => {
    return (<div className="flex w-2 h-2 rounded-full animate-subtle-pulse bg-green-700"/>);
};
