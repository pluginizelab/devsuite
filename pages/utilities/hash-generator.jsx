...
export default function HashGenerator() {
    ...
    const handleSaltChange = useCallback((event) => {
        setSaltInput(event.currentTarget.value);
    }, []);
    const handleSecretKeyChange = useCallback((event) => {
        setSecretKey(event.currentTarget.value);
    }, []);
    const handleAlgorithmChange = useCallback((value) => {
        setAlgorithm(value);
    }, []);
    const handleIterationsChange = useCallback((event) => {
        const value = Number(event.currentTarget.value);
        setIterations(Math.min(value, MAX_ITERATIONS));
    }, []);
    const handleOutputLengthChange = useCallback((event) => {
        setOutputLength(Number(event.currentTarget.value));
    }, []);
    const handleEncodingChange = useCallback((value) => {
        setEncoding(value);
    }, []);
    const hashFunctions = useMemo(() => {
        const resolvedSecretKey = secretKey || crypto.randomBytes(32).toString("hex");
        return {
            ...
            "hmac-sha256": () => generateHash("hmac-sha256", textInput, encoding, resolvedSecretKey),
            "hmac-sha512": () => generateHash("hmac-sha512", textInput, encoding, resolvedSecretKey),
        };
    ...
    return (<main>
      ...
      <section className="max-w-2xl mx-auto mb-12">
        <PageHeader title="Hash Generator" description="Fast, free, open source, ad-free tools."/>
      </section>
      ...
      <section className="container max-w-2xl mb-6">
        <Card className="flex flex-col p-6 hover:shadow-none shadow-none rounded-xl">
          <div>
            <Label>Text</Label>
            <Textarea rows={4} placeholder="Enter text to hash" onChange={handleTextChange} className="mb-6" value={textInput} onKeyDown={(event) => {
            ...
            <div className="flex flex-1 gap-4">
              <div className="flex flex-col flex-1">
                ...
                <Combobox data={algorithmOptions} onSelect={handleAlgorithmChange} value={algorithm}/>
              </div>

              <div className="flex flex-col flex-1">
                <Label>Output Encoding</Label>
                <Combobox data={encodingOptions} onSelect={handleEncodingChange} value={encoding}/>
              </div>
            </div>

            <Divider />

            {algorithm === "pbkdf2" && (<div>
                <Label>Salt (optional)</Label>
                <Input type="text" placeholder="Enter custom salt or leave blank for random" value={saltInput} onChange={handleSaltChange} className="mb-4"/>

                <Label>Iterations</Label>
                <Input type="number" value={iterations} onChange={handleIterationsChange} className="mb-4" min="1000" max={MAX_ITERATIONS}/>

                <Label>Output Length (bytes)</Label>
                <Input type="number" value={outputLength} onChange={handleOutputLengthChange} className="mb-4" min="16" max="128"/>
              </div>)}

            {(algorithm === "hmac-sha256" || algorithm === "hmac-sha512") && (<div>
                <Label>Secret Key (optional)</Label>
                <Input type="text" placeholder="Enter custom secret key or leave blank for random" value={secretKey} onChange={handleSecretKeyChange} className="mb-4"/>
              </div>)}
          </div>
