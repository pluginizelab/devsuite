...
export default function SVGViewer() {
    ...
    const processSVGContent = useCallback((value) => {
        if (value.trim() === "") {
            setBase64(null);
            setStatus("idle");
            return;
    ...
    const handleSVGFileSelect = useCallback((svgContent) => {
        setInput(svgContent);
    ...
    return (<main>
      ...
      <section className="container max-w-2xl mb-12">
        <PageHeader title="SVG Viewer" description="Fast, free, open source, ad-free tools."/>
      </section>
          ...
          <Tabs defaultValue="paste" className="w-full">
            ...
            <TabsContent value="paste" className="space-y-4">
              <div>
                ...
                <Textarea rows={6} placeholder="Paste SVG code here" onChange={handleChange} value={input} aria-label="SVG code input"/>
              </div>
            </TabsContent>
            ...
            <TabsContent value="upload" className="space-y-4">
              <div>
                ...
                <SVGUploadComponent onSVGSelect={handleSVGFileSelect} maxFileSize={2 * 1024 * 1024} // 2MB
    />
                {input && (<div className="mt-4">
                    <Label>Uploaded SVG Code</Label>
                    <Textarea rows={6} value={input} onChange={handleChange} aria-label="Uploaded SVG code" className="mt-2"/>
                  </div>)}
              </div>
            </TabsContent>
            ...
            <div className="w-full">
              {base64 && (<div className="mt-6 border rounded p-4 overflow-auto">
                  <img src={base64} alt="SVG Preview" className="w-full h-auto" onError={() => {
                setStatus("error");
                setBase64(null);
            }}/>
                </div>)}
