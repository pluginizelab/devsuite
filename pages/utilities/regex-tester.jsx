...
import { createRegex, parseFlags, buildPatternWithFlags, getPatternBody, } from "@/components/utils/regex-tester.utils";
...
export default function RegexTester() {
    ...
    const handlePresetSelect = useCallback((presetPattern, presetTestString) => {
        setPattern(presetPattern);
        setTestString(presetTestString);
        const parsedFlags = parseFlags(presetPattern);
        setFlags(parsedFlags);
    }, []);
    const handleCheatSheetInsert = useCallback((syntax) => {
        const patternBody = getPatternBody(pattern) || "";
    ...
    return (<main>
      ...
      <section className="container max-w-4xl mb-12">
        <PageHeader title="Regex Playground" description="Interactive regex tester with pattern explanation, capture groups, and more."/>
      </section>
        ...
        <Card className="flex flex-col p-6 hover:shadow-none shadow-none rounded-xl">
          <div className="space-y-6">
            <div>
              ...
              <RegexPresetPatterns onSelect={handlePresetSelect} selectedPattern={pattern}/>
            </div>
            ...
            <div>
              ...
              <Textarea ref={patternInputRef} rows={1} placeholder="Enter regex pattern here (e.g., /pattern/g)" onChange={(event) => handlePatternChange(event.target.value)} className="mb-3 min-h-0 font-mono" value={pattern}/>
            ...
            <div>
              ...
              <Textarea rows={4} placeholder="Enter test string here" onChange={(event) => setTestString(event.target.value)} value={testString}/>
            </div>
            ...
            <div>
              ...
              <div className="w-full rounded-lg border border-input bg-muted px-3 py-2 text-sm ring-offset-background">
                <div className={result?.startsWith("Match found")
            ...
            {pattern && (<div className="border-t border-border pt-4">
                <button onClick={() => setShowCaptureGroups(!showCaptureGroups)} className="flex items-center gap-2 text-sm font-medium text-muted-foreground hover:text-foreground transition-colors">
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className={`w-4 h-4 transition-transform ${showCaptureGroups ? "rotate-90" : ""}`}>
                    <path fillRule="evenodd" d="M7.21 14.77a.75.75 0 01.02-1.06L11.168 10 7.23 6.29a.75.75 0 111.04-1.08l4.5 4.25a.75.75 0 010 1.08l-4.5 4.25a.75.75 0 01-1.06-.02z" clipRule="evenodd"/>
                  </svg>
                  Capture Groups
                </button>
                {showCaptureGroups && (<div className="mt-3">
                    <RegexCaptureGroupVisualizer pattern={pattern} testString={testString}/>
                  </div>)}
              </div>)}
