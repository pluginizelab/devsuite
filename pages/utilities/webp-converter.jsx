...
import { batchConvertToWebP, downloadWebPZip, formatFileSize, filterSupportedImages, cleanup, } from "@/components/utils/webp-converter.utils";
...
export default function WebPConverter() {
    ...
    const handleFilesSelect = useCallback((selectedFiles) => {
        ...
        const newFileItems = supportedFiles.map((file, index) => ({
            file,
            id: `${Date.now()}-${index}`,
        }));
    ...
    const handleQualityInputChange = useCallback((e) => {
        const value = parseInt(e.target.value);
        const newQuality = Math.max(1, Math.min(100, value));
        setQuality(newQuality);
        localStorage.setItem("webp-converter-quality", newQuality.toString());
    }, []);
    const handleAutoDownloadChange = useCallback((checked) => {
    ...
    const handleConvert = useCallback(async () => {
        ...
        setConversionResults([]);
        try {
            const result = await batchConvertToWebP(files.map((f) => f.file), { quality }, (completed, total) => {
    ...
    return (<main>
      <Meta title="WebP Converter | Free, Open Source & Ad-free" description="Convert images to WebP format with batch processing and quality control. Fast, free, open source, ad-free tools."/>
      <Header />
      <CMDK />

      <section className="container max-w-2xl mb-12">
        <PageHeader title="WebP Converter" description="Convert images to WebP format with batch processing."/>
      </section>

      <section className="container max-w-2xl mb-6">
        <Card className="flex flex-col p-6 hover:shadow-none shadow-none rounded-xl">
          <div>
            <MultiFileUploadComponent maxFileSize={MAX_FILE_SIZE} onFilesSelect={handleFilesSelect} multiple={true} acceptedTypes={["image/*"]}/>
            ...
            {files.length > 0 && (<>
                <div className="flex justify-between items-center mb-4 pt-6">
                  ...
                  <Button variant="outline" size="sm" onClick={clearAllFiles} className="h-9">
                ...
                <div className="max-h-48 overflow-y-auto space-y-2 mb-4">
                  {files.map((item) => (<div key={item.id} className="flex items-center justify-between p-4 border border-border rounded-lg bg-card hover:bg-muted/50 transition-colors">
                      ...
                      <Button variant="ghost" size="sm" onClick={() => removeFile(item.id)} className="h-8 w-8 p-0 ml-3 flex-shrink-0 hover:bg-destructive/10 hover:text-destructive">
                ...
                <div className="mb-6">
                  ...
                  <div className="flex items-center gap-4">
                    <Slider value={[quality]} onValueChange={handleQualityChange} max={100} min={1} step={1} className="flex-1"/>
                    <Input type="number" min="1" max="100" value={quality} onChange={handleQualityInputChange} className="w-20"/>
                  </div>
                ...
                <div className="mb-6">
                  <div className="flex items-center space-x-2">
                    <Checkbox id="auto-download" checked={autoDownload} onCheckedChange={handleAutoDownloadChange}/>
                    <Label htmlFor="auto-download" className="cursor-pointer text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70 m-0">
                ...
                <div className="flex gap-4">
                  <Button className="flex-1" onClick={handleConvert} disabled={isConverting || files.length === 0}>
                    {isConverting ? "Converting..." : "Convert to WebP"}
                  </Button>

                  {conversionResults.length > 0 && (<Button variant="outline" className="flex-1" onClick={handleDownloadAll} disabled={conversionResults.filter((r) => r.success).length === 0}>
                      <DownloadIcon className="h-4 w-4 mr-2"/>
                      Download All
                    </Button>)}
                </div>

                {progress && (<div className="mb-4 py-4">
                ...
                {conversionStats && (<>
                    ...
                    : "images"}{" "}
                          ...
                          {conversionStats.failed > 0 && (<span className="text-red-600 ml-1">
