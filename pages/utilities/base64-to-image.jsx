...
export default function Base64ToImage() {
    ...
    const processBase64 = useCallback((value) => {
        if (value.trim() === "") {
            setImageSrc(null);
            setStatus("idle");
            return;
    ...
    return (<main>
      ...
      <section className="container max-w-2xl mb-12">
        <PageHeader title="Base64 to Image Converter" description="Fast, free, open source, ad-free tools."/>
      </section>
      ...
      <section className="container max-w-2xl mb-6">
        <Card className="flex flex-col p-6 hover:shadow-none shadow-none rounded-xl">
          <div>
            ...
            <Textarea rows={6} placeholder="Paste Base64-encoded image data here (with or without data URI prefix)" onChange={handleChange} value={input} aria-label="Base64 input"/>
          </div>

          {status !== "idle" && (<div className="mt-6 flex text-red-600 text-sm" role="alert">
              {getStatusMessage(status)}
            </div>)}

          <div className="w-full">
            {imageSrc && (<div className="mt-6 border rounded p-4 overflow-auto">
                <Label>Image Preview</Label>
                <img src={imageSrc} alt="Decoded Base64 Image" className="w-full h-auto mt-2" onError={handleImageError}/>
              </div>)}
          </div>
          ...
          <div className="mt-6 text-sm text-muted-foreground">
            <p>
              ...
              <Link href="/utilities/image-to-base64" className="text-primary underline hover:no-underline">
      ...
      <GitHubContribution username="devin-ai-integration" customUrl="https://app.devin.ai"/>
      <CallToActionGrid />
