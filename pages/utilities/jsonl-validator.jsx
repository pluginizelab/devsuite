...
import { parseJsonLines, toJsonArrayString, } from "@/components/utils/jsonl-validator.utils";
...
export default function JsonlValidator() {
    ...
    const handleJumpToIssue = useCallback((lineNumber, columnNumber) => {
    ...
    return (<main>
      <Meta title="JSONL Validator for AI Logs & Event Streams | Free Tool" description="Validate AI dataset rows, observability logs, and event stream payloads with line-level error jumps and clean JSON export."/>
      <Header />
      <CMDK />

      <section className="container max-w-4xl mb-12">
        <PageHeader title="JSONL Validator" description="Validate JSONL for AI datasets, logs, and event streams."/>
      </section>
          ...
          <div>
            ...
            <div className="mb-6 rounded-lg border border-input bg-muted overflow-hidden">
              <div className="grid grid-cols-[56px_1fr]">
                <div className="border-r border-input bg-muted/80" aria-hidden="true">
            ...
            return (<div key={lineNumber} className={cn("h-6 px-2 flex items-center justify-end gap-1 text-xs font-mono", error
                ...
                <Textarea ref={textareaRef} rows={12} wrap="off" spellCheck={false} placeholder='{"id":1,"event":"start"}' onChange={handleInputChange} onScroll={handleInputScroll} className="h-72 resize-none rounded-none border-0 bg-transparent font-mono leading-6 focus-visible:ring-0 focus-visible:ring-offset-0" value={input} aria-invalid={hasErrors} aria-describedby={hasErrors ? "jsonl-issues" : undefined}/>
              </div>
            </div>
            ...
            {hasErrors && (<div id="jsonl-issues" className="rounded-xl border border-input bg-muted/40 p-3 mb-6">
                <p className="text-sm font-medium text-foreground flex items-center gap-2">
                  <span aria-hidden="true" className="inline-block h-2 w-2 rounded-full bg-destructive/80"/>
                  Invalid rows found. Select an issue to jump to the exact line.
                </p>
                <ul className="mt-3 space-y-2">
                  {result.errors.slice(0, MAX_ISSUES_SHOWN).map((error) => (<li key={`${error.lineNumber}-${error.message}`}>
                      <button type="button" onClick={() => handleJumpToIssue(error.lineNumber, error.columnNumber)} className={cn("w-full rounded-lg border px-3 py-2 text-left transition-colors duration-150 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring", activeIssueLine === error.lineNumber
                ...
                {result.invalidLines > MAX_ISSUES_SHOWN && (<p className="mt-2 text-xs text-muted-foreground">
                    Showing first {MAX_ISSUES_SHOWN} issues.
                  </p>)}
              </div>)}

            {topKeys.length > 0 && (<div className="rounded-lg border border-input bg-muted/50 p-3 mb-6">
                <p className="text-sm font-medium mb-2">Top keys</p>
                <div className="flex flex-wrap gap-2">
                  {topKeys.map(([key, count]) => (<span key={key} className="text-xs rounded-full bg-background border border-input px-2 py-1">
                      {key}: {count}
                    </span>))}
                </div>
              </div>)}
            ...
            <Textarea value={output} rows={10} readOnly className="mb-4 font-mono" placeholder="Valid JSONL rows will appear here as a JSON array."/>

            <Button variant="outline" onClick={() => handleCopy(output)} disabled={!output}>
              {buttonText}
            </Button>
          </div>
        </Card>
...
function MetricCard({ label, value }) {
    return (<div className="rounded-lg border border-input bg-muted/40 p-3">
      <p className="text-xs text-muted-foreground">{label}</p>
      <p className="text-lg font-semibold">{value}</p>
    </div>);
}
function Divider() {
    return (<div className="my-6 border-t border-input" role="separator" aria-orientation="horizontal"/>);
}
function formatLinePreview(lineContent) {
