...
import { convertToHex, convertToRGB, isRGBValueValid, isValidHex, toAndroidColor, toCss, toIOS, } from "@/components/utils/hex-to-rgb.utils";
...
export default function HEXtoRGB(props) {
    ...
    const handleRGBChange = (key, event) => {
    ...
    return (<main>
      ...
      <section className="container max-w-2xl mb-12">
        <PageHeader title={pageTitle} description="Free, Open Source & Ad-free"/>
      </section>
      ...
      <section className="container max-w-2xl mb-6">
        <Card className="flex flex-col p-6 hover:shadow-none shadow-none rounded-xl">
          <div>
            ...
            "flex-col-reverse": props.invertOrder,
              ...
              <div>
                ...
                <div className={cn("flex items-center")}>
                  <Input placeholder="#000000" onChange={handleHexChange} className="h-8 text-sm mr-4" value={hex} maxLength={7}/>
                  <div className="w-8 h-8 border rounded-full block flex-none" style={{
            ...
            return (<div key={colorKey}>
                      <Label className="min-w-12 block mr-1">
                        {colorNameMap[colorKey]}
                      </Label>
                      <Input className="h-8 text-sm" type="number" value={rgb[colorKey]} onChange={(event) => handleRGBChange(colorKey, event)} onFocus={(e) => e.currentTarget.select()} placeholder="0" min="0" max="255"/>
                    </div>);
        })}
              </div>
            </div>
            ...
            <CodeSnippetRow label="CSS" rgb={rgb} convertFunction={(rgb) => toCss(rgb)}/>
            <CodeSnippetRow label="Obj C" rgb={rgb} convertFunction={(rgb) => toIOS(rgb, "c")}/>
            <CodeSnippetRow label="Swift" rgb={rgb} convertFunction={(rgb) => toIOS(rgb, "swift")}/>
            <CodeSnippetRow label="Android" rgb={rgb} convertFunction={(rgb) => toAndroidColor(rgb)}/>
          </div>
        </Card>
      </section>
