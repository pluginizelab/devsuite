...
import { pxToRem, remToPx, pxToVw, vwToPx, pxToVh, vhToPx, pxToVmin, vminToPx, pxToVmax, vmaxToPx, } from "@/components/utils/css-units-converter.utils";
...
export default function CSSUnitsConverter() {
    ...
    const needsContainerInput = ["vw", "vh", "vmin", "vmax"].includes(unit.from) ||
    ...
    const convertUnits = useCallback(() => {
        ...
        const conversionKey = `${unit.from}-${unit.to}`;
        ...
        if (conversionFunction) {
            if (["px-vw", "vw-px"].includes(conversionKey)) {
                ...
                convertedValue = conversionFunction(value, containerWidthValue, containerHeightValue);
            }
    ...
    return (<main>
      <Meta title="CSS units converter | Free, Open Source & Ad-free" description="Easily convert px to rem with our simple CSS unit converter"/>
      <Header />
      <CMDK />
      <section className="container max-w-2xl mb-12">
        <PageHeader title="CSS Units Converter" description="Fast, free, open source, ad-free tools."/>
      </section>
      <section className="container max-w-2xl mb-6">
        <Card className="flex flex-col p-6 hover:shadow-none shadow-none rounded-xl">
          <div>
            <Label>Input Value</Label>
            <Input className="mb-6" value={htmlInput} onChange={handleInputChange}/>
            <div className="mb-6 flex w-full gap-4">
              <div className="flex flex-1 flex-col">
                <Label>From Unit</Label>
                <Combobox data={unitOptions} value={unit.from} onSelect={(value) => setUnit({ from: value, to: unit.to })}/>
              </div>
              ...
              <div className="flex flex-1 flex-col">
                <Label>To Unit</Label>
                <Combobox data={unitOptions} value={unit.to} onSelect={(value) => setUnit({ from: unit.from, to: value })}/>
              </div>
            </div>
            {needsContainerInput && (<div className="mb-6 flex gap-4">
                <div className="flex flex-1 flex-col">
                  <Label>Container Width (px)</Label>
                  <Input type="number" className="h-[32px]" value={widthInput} onChange={(e) => setWidthInput(e.target.value)}/>
                </div>
                <div className="flex flex-1 flex-col">
                  <Label>Container Height (px)</Label>
                  <Input type="number" className="h-[32px]" value={heightInput} onChange={(e) => setHeightInput(e.target.value)}/>
                </div>
              </div>)}
...
const conversionFunctionMapper = {
    "px-rem": pxToRem,
    "rem-px": remToPx,
    "px-vw": pxToVw,
    "vw-px": vwToPx,
